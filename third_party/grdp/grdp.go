...
func (g *RdpClient) SetClipboard(clipboard cliprdr.Clipboard) {
	if clipboard != nil {
		g.clipboard = cliprdr.NewClient(clipboard)
	}
}

func (g *RdpClient) ClipboardChanged() {
...
func (g *RdpClient) Login(domain string, user string, password string) error {
	...
	if g.clipboard != nil {
		g.channels.Register(g.clipboard)
